...
(function () {
   ...
   *
   * Chromium hands a folder drop over through `webkitGetAsEntry`, which
   * answers synchronously; Firefox has no such method and offers
   * `getAsFileSystemHandle()` instead, which answers with a promise. Both are
   * accepted here, and the walker below settles whichever it is given.
   */
  function droppedEntries(transfer) {
    ...
    for (const item of Array.from(transfer.items)) {
      ...
      if (typeof item.getAsFileSystemHandle === 'function') {
        out.push(item.getAsFileSystemHandle());
      }
    }
    return out;
  }
  ...
  async function collectDropped(entries, plain) {
    ...
    const walk = async (entry, depth, under) => {
      ...
      // Firefox's handle comes back as a promise; Chromium's entry is ready.
      if (entry && typeof entry.then === 'function') {
        try { entry = await entry; } catch (error) { return; }
      }
      if (!entry) return;
      const file = entry.isFile
        ? await fileOf(entry)
        : entry.kind === 'file'
          ? await entry.getFile().catch(() => null)
          : null;
      if (file) {
      ...
      if (!entry.isDirectory && entry.kind !== 'directory') return;
      const inside = under ? `${under}/${entry.name}` : entry.name;
      if (entry.isDirectory) {
        const reader = entry.createReader();
        // readEntries hands back a batch at a time and an empty one at the
        // end, so it has to be asked until it says there is no more.
        for (;;) {
      ...
      for await (const [, child] of entry.entries()) {
        await walk(child, depth + 1, inside);
      }
    };
  ...
  /**
   * Whether the skin that is on is what supplies a texture, rather than the
   * car itself.
   *
   * The same question `decodeTexture` asks before it reaches for the embedded
   * picture, asked here of a request that has not been decoded yet — a skin
   * replaces the texture of a name and a material names one, so the answer is
   * in the two names and needs neither picture.
   */
  const fromSkin = (request) => !!(wearing && request
    && wearing.images.has(baseName(request.path || request.name)));
   ...
   *
   * `flat` for a picture that is one colour from corner to corner, which as a
   * grain is not a grain at all: nothing about it differs from its own
   * average, so there is nothing for it to say. A third of the detail maps in
   * the 67 cars to hand are that — `NULL.dds`, `PURE_RED.dds`, a numbered
   * `2.dds` — the slot filled in and never authored. Taken as a grain, the
   * greys among them come out as the nothing they are, and the 55 that are a
   * flat saturated colour repaint whatever wears them: a Jaguar Mk2 whose
   * paint names a sixteen-pixel square of pure red draws red under every skin
   * it has, liveries and previews notwithstanding.
   *
   * Which is why the flag is separate from the number rather than standing in
   * for it: whether a flat picture is a placeholder or the paint is not a
   * question about the picture, and the caller is the one that can tell.
   */
  function detailScale(image, edge = 32) {
    try {
      ...
      return { scale: mean > 0.02 ? 1 / mean : 1, flat };
    } catch (error) {
      // unreadable pixels: take it as it comes, and as a grain
      return { scale: 1, flat: false };
    }
  }
  ...
  async function resolveTextures(palette) {
    /* The tiling each material states, put back before its grain is looked at
     * again.
     *
     * A picture refused as no grain takes the tiling with it, since that is
     * what the export asks before baking a grain into a picture. But this runs
     * again every time a skin goes on or comes off, and a slot switched off
     * last time is one the new picture would never be asked for — so a skin
     * bringing the paint for a slot the car had filled with a placeholder
     * would arrive too late to be looked at at all. What the file said is
     * remembered once and restored each pass, and the refusal is made afresh
     * against whichever picture is actually there. */
    for (const material of palette) {
      if (material.statedDetailTiling === undefined) {
        material.statedDetailTiling = material.detailTiling;
      }
      material.detailTiling = material.statedDetailTiling;
    }
    ...
    for (const material of palette) {
      ...
      const read = material.detailLayer >= 0 ? scales[material.detailLayer] : null;
       ...
       *
       * Unless the skin that is on brought it, in which case the flat colour
       * is the paint and dropping it is what leaves the car white. A BMW Z3
       * states its paint nowhere — no `cm_skin.json`, and seven
       * `[Material_CarPaint*]` sections that name their skins and state no
       * colour — because the colour is the detail map: `body` is a
       * `ksPerPixelMultiMap` over one grey ambient-occlusion sheet every skin
       * shares, and each skin lays its own `metal_detail.dds` over it. Six of
       * the twelve are a single repeated block — pure red for Hellrot, black
       * for Schwarz, #cdae29 for Dakargelb — and refused as placeholders all
       * six draw the same white as the sheet underneath.
       *
       * So where a flat picture came from is what says what it is. The car's
       * own is the slot filled in and never authored, and is the same picture
       * under every skin it has — a Jaguar Mk2's sixteen-pixel square of pure
       * red, which its four skins do not replace and three of which are pale
       * green. One the worn skin carries is the one thing that changes when
       * the skin does, which is the definition of the paint. */
      const placeholder = read && read.flat
        && !fromSkin(material.textures && material.textures.detail);
      if (placeholder) {
        material.detailLayer = -1;
        material.detailTiling = 0;
      }
      material.detailScale = read ? read.scale : 1;
    }
    return {
      images: base.images,
