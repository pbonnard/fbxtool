...
const FbxMax = (function () {
  ...
  const SHADERS = new Map([
     ...
     *
     * It keeps two indices side by side and only one of them shapes the
     ...
     * reflectance of window glass, and the Grecale's chrome, its polished
     * steel and its wing mirrors all drew as dark plastic.
     *
     ...
     * for the steel, 8 and 6 for the two irons — while `CoronaMtlPb|ior` is
     * the 1.52 sitting at 183. */
    ['coronamtl', {
   ...
   */
  function appearanceOf(params, layout, diffuse = null, specular = null) {
    const stoodIn = { diffuse, specular };
    const at = (which, key) => {
      // These stand in for the colours the block itself holds, for a material
      // whose slot is filled by a colour rather than a picture: the one beside
      // it is then a placeholder. They go in before the level is applied,
      // since a channel switched off is switched off whichever colour it was
      // given.
      if (key === 'colour' && stoodIn[which]) return stoodIn[which];
  ...
  const FALLOFF_MAP = 'falloff';
  const FALLOFF_NEAR = 0;
  ...
  function colourUnder(view, entities, index, depth = 0, seen = new Set()) {
     ...
     * angle, and the end that matters is the near one — parameter 0 — because
     * that is what facing the camera means, and every number this reader hands
     * the renderer is a facing one. It is a fallback and not a rule: a Falloff
     * with a map in its near slot shows that map, so what lies beneath is
     * asked first. */
    if (String(entity.cls.name || '').trim().toLowerCase() === FALLOFF_MAP) {
      for (const ref of [index, ...entity.refs]) {
        if (ref >= entities.length) continue;
        for (const p of paramsOf(view, entities[ref])) {
          if (p.id === FALLOFF_NEAR && p.colour !== undefined) return p.colour;
        }
      }
    }
    return null;
  ...
  /* And where one keeps it as a map slot instead, which is a stronger
   * statement than a number: a CoronaLayeredMtl puts the amount of its first
   * layer at slot 11, and all three of the Grecale's paints fill it with a
   * Falloff that is black facing you and 0.86 at grazing. That is a coat seen
   * along the edges of a panel and nowhere else — so facing the camera there
   * is no coat, which is also what 3ds Max's own export makes of these three:
   * it writes the base coat's reflection and shininess and says nothing about
   * a layer at all. */
  const COAT_AMOUNT_SLOT = { coronalayeredmtl: 11 };
  ...
  /* Slot 1 is the reflection, and it is read for the same reason the diffuse
   * is: a filled slot makes the colour beside it a placeholder. Car paint is
   * where this shows. All three of the Grecale's layered paints put a Falloff
   * there — black facing you, white at grazing, with the paint's own
   * CoronaColor beneath it — which is an artist drawing the Fresnel curve by
   * hand. The flat white parameter left in the block is not what reflects:
   * read it and the whole body comes out chrome, and the gold reflects white
   * instead of gold. */
  const MAP_SLOTS = {
  ...
  function readMaterial(view, bytes, entities, index, assets) {
    ...
    let reflected = null;           // and the same for the reflection slot
    const named = String(entity.cls.name || '').trim().toLowerCase();
    const atAmount = COAT_AMOUNT[named];
    ...
    const atSlot = COAT_AMOUNT_SLOT[named];
    if (atSlot !== undefined) {
      const shown = colourUnder(view, entities,
        keyedMaps(entities, entity, 'block')[atSlot]);
      if (shown) amount = Math.min(1, Math.max(0, Math.max(...shown)));
    }
    const slots = MAP_SLOTS[named];
    if (slots) {
      ...
      if (slots.specular !== undefined) {
        reflected = colourUnder(view, entities, keyed[slots.specular]);
      }
    }
    ...
    while (queue.length && walked.size < 64) {
      ...
      if (layout && !look) {
        look = appearanceOf(params, layout, painted, reflected);
